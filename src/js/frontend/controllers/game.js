...
fullcourt.controller("GameController", function($http, $timeout, $scope, SVGBuilder, ChartBuilder) {
    
    const renderAllGraphs = function(response, status, headers, config) {
        ...
        const deviationLegends =  document.querySelectorAll('svg.legend');
        for (let legend of Array.from(deviationLegends)) {
            ChartBuilder.buildDeviationLegend(legend);
        }
        
        const away_score_data = {
        ...
        for (var player of Array.from(response.box_score.away_stats)) {
            if (player.minutes < 1) { continue; }
            if (player.points > 0) {
        ...
        for (player of Array.from(response.box_score.home_stats)) {
            if (player.minutes < 1) { continue; }
            if (player.points > 0) {
                display_name = player.first_name[0] + '. ' + player.last_name;
        ...
        return ChartBuilder.buildBarGraph(
    ...
    return $http.get(`/data${location.pathname}?t=${document.getElementById('graphs').dataset.token}`)
